...
def get_test_roidb(path):
    test_data = cPickle.load(open(path + 'test_data_5fps_768.pkl'))
    return test_data
...
if __name__ == '__main__':
    ...
    path = './preprocess/charades/'
    imdb = get_test_roidb(path)

    if not cfg.TEST.HAS_RPN:
        imdb.set_proposal_method(cfg.TEST.PROPOSAL_METHOD)

    # set the threshold for charades (1/157=0.0064)                                              
    test_net(net, imdb, max_per_image=args.max_per_image, thresh=0.0064, vis=args.vis)
